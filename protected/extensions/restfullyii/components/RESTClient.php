...
class RESTClient extends CComponent
{
    ...
    private function _call($method, $uri, $params = array(), $format = NULL)
    {
    	$this->format($format);
    	$this->_set_headers();
    	$params = $this->{"_".$this->format."_encode"}($params);
        if ($method=='get')
        {
        	$uri .= '?'.$params;
            $params= null;
        ...
        /**
         * 外部接口调用日志记录 chenjinlong 20131118
         */
        /*LBSLogger::logging("RestInvoking",'Outer-API','外部数据接口调用','',array(
            'url' => $uri,
            'http_method' => $method,
            'get_parameter' => base64_decode($params),
        ...
        // Format and return
		return $this->{"_".$this->format."_decode"}($response);
    }
    ...
    // Encode as JSON
    private function _json_encode($string)
    {
    ...
    private function _serialize_encode($string)
    {
    	return serialize(trim($string));
    }

    // Decode as Serialized array
    private function _serialize_decode($string)
    {
    ...
    // Encode raw Encrypt
    private function _encrypt_encode($string)
    {
    	return base64_encode(json_encode($string));
    }

    // Decode raw Encrypt
    private function _encrypt_decode($string)
    {
