 ...
 * Date: 14/01/18
 * Time: 
 * Description: ReleaseSubjectClsMod.php
 */
...
class ReleaseSubjectClsMod extends ReleaseCustom
{
     ...
     * 执行发布操作
     *
     * @author wenrui 20140118
     * @return bool
     */
    public function runRelease()
    {
        //查询配置的容纳产品最大值
        $maxShowRec = $this->getAdPositionCountByType('special_subject');
        //查询推广开始日期为当天的出价列表
        $bidProductArray = $this->_releaseProductMod->getProductArr('special_subject');
        ...
        foreach ( $bidProductGroups as $key => $bidProductGroup ) {
            ...
            if(!empty($imperfect)||!empty($feedBackResultArr)){
                ...
                CommonSysLogMod::log(date('Y-m-d H:i:s') . __CLASS__, '本次专题页推广结束', 11, 'wenrui',0,0,'');
            }
        }
    }
...
//                $index = count($tmpArr[$startCityCode]);
//                $tmpArr[$startCityCode][$productId] = array_merge($tmpArr[$startCityCode][$productId], array(
//                    'ranking' => $index,));
                //结果接口格式结构化
