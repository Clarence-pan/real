 ...
 * Date: 12/20/12
 * Time: 2:47 PM
 * Description: ReleaseIndexCoreMod.php
 */
...
class ReleaseIndexCoreMod extends ReleaseCustom
{
    ...
    public function runRelease()
    {
        //do release job
        //查询配置的容纳产品最大值
        $maxShowRec = $this->getAdPositionCountByType('index_chosen');
        //查询推广开始日期为当天的出价列表
        $bidProductArray = $this->_releaseProductMod->getProductArr('index_chosen');
        //将需要推广的产品按推广城市位置分组
        ...
        foreach ( $bidProductGroups as $key => $bidProductGroup ) {
            ...
            if(!empty($imperfect)||!empty($feedBackResultArr)){
                ...
                CommonSysLogMod::log(date('Y-m-d H:i:s') . __CLASS__, '本次首页推广结束', 11, 'wenrui',0,0,'');
            }
        }
    }
