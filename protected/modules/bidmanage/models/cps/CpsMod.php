...
//			}
	...
	public function getCpsAgreement($param) {
		...
		// 查询CPS协议
		$result = "<div style=\"text-indent:5em;font-size:24;font-weight:bold\">2014年途牛旅游网CPS推广协议</div><br/>".
					...
					"2、用户同意了本协议或使用途牛网CPS服务，即表示用户已接受途牛网CPS服务的现有功能和规则，也同意途牛有权无须经用<br/>" .
					"户同意或提前做出告示而直接对途牛网CPS服务的功能、规则进行变更、升级、改进、修改，用户也愿意为此支付相关服务费用。<br/>".
					"3、用户在任何情况下，包括但不限于通过与途牛网的沟通、诉讼或其它方式，获知途牛网未在途牛网公布的信息，未经途牛书面同<br/>" .
					"意情况下，用户不得将该信息告知其他任何第三人。</div>".
					"<div style=\"font-size:16;width:660px\">三、本协议的确认、接受、修订<br/>".
					"1、本协议内容包括协议正文及所有与途牛网CPS服务相关的已经发布的或将来可能发布的各类规则。上述内容为本协议不可分割的<br/>" .
					"一部分，与本协议正文具有同等法律效力。用户在使用途牛网CPS服务时，应予遵守。<br/>".
					"2、无论用户称事实上是否阅读了本协议，只要用户点击接受本协议并开启CPS服务流程，则仍表示用户同意接受本协议约束。<br/>".
					"3、鉴于互联网相关技术、市场、业务模式等等的发展迅速，用户同意，途牛有权随时根据中华人民共和国有关法律、法规的变化、互<br/>" .
					"联网的发展以及公司经营状况和经营策略的调整等不定时地制定、修改本协议及各类规则，如有修订，途牛将在用户NB系统、<br/>".
					"邮箱、途牛网公告等渠道通知于用户。经修订的协议、规则一经公布，则立即自动生效，且成为本服务协议不可分割的一部分。如用户<br/>" .
					"不同意途牛的上述修订，则用户必须立即停止使用本协议下各项服务。用户登录或继续使用途牛网CPS服务，则表示用户接受经修订的<br/>".
					"协议、规则。<br/>".
					...
					"2、用户应妥善保管好自身的用户名及其密码，并对该用户名下的一切操作行为负责。若发现任何非法使用用户名及其密码的，用户应立<br/>" .
					"即以有效方式通知途牛。<br/>".
